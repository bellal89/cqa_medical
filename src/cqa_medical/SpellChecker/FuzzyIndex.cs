...
namespace cqa_medical.SpellChecker
{
	internal class FuzzyIndex
	{
		...
		private Dictionary<string, string> GetFuzzyDictionary(IEnumerable<LevensteinInfo> levensteinInfos)
		{
			...
			File.WriteAllLines("__Dictionary_words_to_variants.txt", levensteinInfos.GroupBy(info => info.GetDictionaryWord(), (key, infos) => new { dictionaryWord = key, infos }).OrderByDescending(it => wordFrequencies.ContainsKey(it.dictionaryWord) ? wordFrequencies[it.dictionaryWord] : 0).Select(it => it.dictionaryWord + " (" + (wordFrequencies.ContainsKey(it.dictionaryWord) ? wordFrequencies[it.dictionaryWord] : 0) + ")\t" + String.Join(", ", it.infos.OrderByDescending(i => wordFrequencies.ContainsKey(i.GetWord()) ? wordFrequencies[i.GetWord()] : 0).Select(i => i.GetWord() + " (" + (wordFrequencies.ContainsKey(i.GetWord()) ? wordFrequencies[i.GetWord()] : 0) + ")"))));
