...
namespace cqa_medical.Statistics
{
	class DesToMedTableEvaluator
	{
		...
		public Dictionary<string, List<string>> GetSynonimsToDeseeasesRelation ()
		{
			var synToIds = new Dictionary<string, List<string>>();
			foreach (var des in mkb10Deseases)
			{
				foreach (var syn in des.Synonyms)
				{
					if(!synToIds.ContainsKey(syn))
						synToIds[syn] = new List<string>();
					synToIds[syn].Add(des.Name);
				}
			}
			return synToIds;
		}
